...
一个精心整理的现代桌游规则参考网站，覆盖 39 款游戏（含扩展/变体），支持中英双语、交互式决策树、训练器、LLM 对话查询，纯静态站点部署到 GitHub Pages。

## 功能特性

- **39 款游戏规则**：经过网络验证的中英双语完整规则
- **33 款交互式决策树**：分步交互流程，含侧边栏目录导航
...
1. **构建时同步读取文件** — `fs.readFileSync` 仅在 `next build` 时执行，39 款游戏绰绰有余
