...
一个精心整理的现代桌游规则参考网站，覆盖 **55** 款游戏（含扩展/变体），支持中英双语、交互式决策树、训练器、**BBGE 在线对局**、LLM 对话查询，纯静态站点部署到 GitHub Pages。

## 功能特性

- **55 款游戏规则**：经过网络验证的中英双语完整规则
- **49 款交互式决策树**：分步交互流程，含侧边栏目录导航
- **12 款自动计分器**：`cabo-multi` / `sea-salt-multi` / `just-wild-multi` / `nimmt-multi` / `category` / `feature-calc` / `card-type` 等，支持多人同屏计分，localStorage 持久化
...
> **维护提示**：新增/调整游戏后，请同步更新下方「游戏清单」与文首数量（当前：`55` 游戏 / `49` 决策树 / `12` 计分 / `5` 训练器 / `1` 计算器 / `5` BBGE 对局）。可用 `node scripts/print-project-stats.mjs` 快速核对。BBGE 行为变更请同步 [`docs/games/<slug>.md`](docs/games/)。
