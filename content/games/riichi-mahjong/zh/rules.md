...
在《日本麻将》中，你要同时兼顾速度、防守和役种。你的目标是和出有役的合法牌，并在整场结束时分数高过其他人。
<!-- rule-section: tiles -->
## 牌具
...
- <!-- rule-item: flow-yaku-beginner -->
  **新手常用役**
  ...
  1. **立直**（1 番）：门清听牌就能宣言
  2. **断幺**（1 番）：不用 1、9 和字牌
  3. **役牌**（1 番）：三元牌/场风/自风的刻子
  4. **平和**（1 番）：全顺子 + 好型听牌
  5. **门前清自摸**（1 番）：门清状态自摸
  
  **小技巧**：立直 + 门前清自摸 + 一发 + 宝牌 = 轻松满贯（8000 点）！
