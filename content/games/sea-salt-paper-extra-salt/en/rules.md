# Sea Salt & Paper: Extra Salt Rules
<!-- rule-section: overview -->
## Overview

Extra Salt is a mini expansion for Sea Salt & Paper, adding **8 new cards** with **5 new effects** to the base game. It cannot be played standalone — requires the base game. Cards are marked with a star symbol for easy identification.
<!-- rule-section: game-objective -->
## Game Objective

In Sea Salt & Paper: Extra Salt, you are using extra sea creatures alongside the base game. Your goal is to turn their new effects into better sets and win under the base game’s scoring.
<!-- rule-section: setup -->
## Setup

Shuffle all 8 expansion cards into the base game deck. No other rule changes are needed.
<!-- rule-ui: tabs -->
<!-- rule-section: new-cards -->
## New Cards

![Jellyfish+Swimmer locks opponents; Lobster looks at 5](/images/rules/sea-salt-paper-extra-salt/new-duos.svg)

![Starfish trio, Seahorse collector, Crab multiplier](/images/rules/sea-salt-paper-extra-salt/specials.svg)
<!-- rule-ui: tabs -->
<!-- rule-section: duo-cards -->
### Duo Cards
<!-- rule-section: jellyfish-swimmer -->
#### Jellyfish + Swimmer (水母 + 泳客)
- **Score:** 1 point for each Jellyfish + Swimmer combination
- **Effect:** On their next turn, opposing players can only draw the first card from the deck. They cannot play any cards or end the round.
<!-- rule-section: lobster-crab -->
#### Lobster + Crab (龙虾 + 螃蟹)
- **Score:** 1 point for each Lobster + Crab combination
- **Effect:** The player takes the first 5 cards from the deck, adds one of them to their hand, then returns the other 4 to the deck and shuffles it.
<!-- rule-ui: tabs -->
<!-- rule-section: special-cards -->
### Special Cards
<!-- rule-section: starfish -->
#### Starfish (海星)
- **How it works:** If a player has a duo and a Starfish card in their hand, they can form a **trio** and place all three cards in front of them.
- **Score:** The Starfish adds 2 points to the duo (therefore, the trio is worth **3 points total**).
- **Effect:** Cancels the effect of the duo cards placed with the Starfish. You only get the points, not the duo's ability.
<!-- rule-section: seahorse -->
#### Seahorse (海马)
- **How it works:** The player can use the Seahorse to replace a **missing Collector card** (octopus, shell, penguin, or sailor).
- **Restrictions:**
  - You must already have **at least one card** of that collection type in your hand.
  - You **cannot gain more points** than the maximum indicated on the matching Collector card.
<!-- rule-section: crab-multiplier -->
#### Crab Multiplier (螃蟹大军)
- **Score:** 1 point per Crab card you have
- **Note:** This card itself does NOT count as a Crab card.
<!-- rule-section: card-count-summary -->
## Card Count Summary

![Use the count table; duos still 2; STOP still works](/images/rules/sea-salt-paper-extra-salt/counts.svg)
| Card | Quantity | Type |
|------|:--------:|------|
| Jellyfish | 2 | Duo (pairs with Swimmer) |
| Lobster | 1 | Duo (pairs with Crab) |
| Starfish | 3 | Triple combo |
| Seahorse | 1 | Wild collector |
| Crab Multiplier | 1 | Score multiplier |
| **Total** | **8** | |
<!-- rule-section: strategy-tips -->
## Strategy Tips

- Lobster + Crab lets you see 5 cards and keep the best one — huge selection advantage
- Jellyfish is devastating when timed before an opponent plans to declare end of round
- Crab Multiplier synergizes with collecting multiple Crab pairs
- Starfish is a trade-off: the trio scores 3 points total but you lose the duo's effect — worth it if the duo effect isn't useful at that moment
- Seahorse is most valuable late-game to complete a collector set you've already started
<!-- rule-section: topic-guide -->
## Quick reference
Jump directly to the rule topic you need.

- <!-- rule-item: flow-setup -->
  **Mix in Extra Salt**

  Shuffle the eight star-marked expansion cards into the Sea Salt & Paper base-game deck. All base-game turn and end-round rules remain unchanged.

  Related topics: 
  - [Duo effects](#flow-duos)
  - [Special cards](#flow-specials)

- <!-- rule-item: flow-duos -->
  **Resolve new duo effects**

  Jellyfish plus Swimmer locks opponents to a top-deck draw next turn. Lobster plus Crab reveals five cards, lets you keep one, then returns and shuffles the other four.

  ![Jellyfish+Swimmer locks opponents; Lobster looks at 5](/images/rules/sea-salt-paper-extra-salt/new-duos.svg)

  Related topics: 
  - [Special cards](#flow-specials)
  - [Card counts](#flow-counts)

- <!-- rule-item: flow-specials -->
  **Choose a special-card payoff**

  Starfish turns a duo into a three-card, three-point set but cancels that duo's effect. Seahorse fills a missing collector card, while Crab Multiplier scores one point per other Crab.

  ![Starfish trio, Seahorse collector, Crab multiplier](/images/rules/sea-salt-paper-extra-salt/specials.svg)

  Related topics: 
  - [Card counts](#flow-counts)
  - [Back to setup](#flow-setup)

- <!-- rule-item: flow-counts -->
  **Know the eight new cards**

  The expansion adds two Jellyfish, one Lobster, three Starfish, one Seahorse, and one Crab Multiplier. The normal duo and STOP rules still apply.

  ![Use the count table; duos still 2; STOP still works](/images/rules/sea-salt-paper-extra-salt/counts.svg)

  Related topics: 
  - [Play the duos](#flow-duos)
  - [Back to setup](#flow-setup)
<!-- rule-section: topic-guide-2 -->
## Quick reference
Jump directly to the rule topic you need.

- <!-- rule-item: flow-setup -->
  **Mix in Extra Salt**

  Shuffle the eight star-marked expansion cards into the Sea Salt & Paper base-game deck. All base-game turn and end-round rules remain unchanged.

  Related topics: 
  - [Duo effects](#flow-duos)
  - [Special cards](#flow-specials)

- <!-- rule-item: flow-duos -->
  **Resolve new duo effects**

  Jellyfish plus Swimmer locks opponents to a top-deck draw next turn. Lobster plus Crab reveals five cards, lets you keep one, then returns and shuffles the other four.

  ![Jellyfish+Swimmer locks opponents; Lobster looks at 5](/images/rules/sea-salt-paper-extra-salt/new-duos.svg)

  Related topics: 
  - [Special cards](#flow-specials)
  - [Card counts](#flow-counts)

- <!-- rule-item: flow-specials -->
  **Choose a special-card payoff**

  Starfish turns a duo into a three-card, three-point set but cancels that duo's effect. Seahorse fills a missing collector card, while Crab Multiplier scores one point per other Crab.

  ![Starfish trio, Seahorse collector, Crab multiplier](/images/rules/sea-salt-paper-extra-salt/specials.svg)

  Related topics: 
  - [Card counts](#flow-counts)
  - [Back to setup](#flow-setup)

- <!-- rule-item: flow-counts -->
  **Know the eight new cards**

  The expansion adds two Jellyfish, one Lobster, three Starfish, one Seahorse, and one Crab Multiplier. The normal duo and STOP rules still apply.

  ![Use the count table; duos still 2; STOP still works](/images/rules/sea-salt-paper-extra-salt/counts.svg)

  Related topics: 
  - [Play the duos](#flow-duos)
  - [Back to setup](#flow-setup)
<!-- rule-section: topic-guide-3 -->
## Quick reference
Jump directly to the rule topic you need.

- <!-- rule-item: flow-setup -->
  **Mix in Extra Salt**

  Shuffle the eight star-marked expansion cards into the Sea Salt & Paper base-game deck. All base-game turn and end-round rules remain unchanged.

  Related topics: 
  - [Duo effects](#flow-duos)
  - [Special cards](#flow-specials)

- <!-- rule-item: flow-duos -->
  **Resolve new duo effects**

  Jellyfish plus Swimmer locks opponents to a top-deck draw next turn. Lobster plus Crab reveals five cards, lets you keep one, then returns and shuffles the other four.

  ![Jellyfish+Swimmer locks opponents; Lobster looks at 5](/images/rules/sea-salt-paper-extra-salt/new-duos.svg)

  Related topics: 
  - [Special cards](#flow-specials)
  - [Card counts](#flow-counts)

- <!-- rule-item: flow-specials -->
  **Choose a special-card payoff**

  Starfish turns a duo into a three-card, three-point set but cancels that duo's effect. Seahorse fills a missing collector card, while Crab Multiplier scores one point per other Crab.

  ![Starfish trio, Seahorse collector, Crab multiplier](/images/rules/sea-salt-paper-extra-salt/specials.svg)

  Related topics: 
  - [Card counts](#flow-counts)
  - [Back to setup](#flow-setup)

- <!-- rule-item: flow-counts -->
  **Know the eight new cards**

  The expansion adds two Jellyfish, one Lobster, three Starfish, one Seahorse, and one Crab Multiplier. The normal duo and STOP rules still apply.

  ![Use the count table; duos still 2; STOP still works](/images/rules/sea-salt-paper-extra-salt/counts.svg)

  Related topics: 
  - [Play the duos](#flow-duos)
  - [Back to setup](#flow-setup)

<!-- rule-section: topic-guide-4 -->
## Quick reference
<!-- rule-ui: sidebar -->

Jump directly to the rule topic you need.

- <!-- rule-item: flow-setup -->
  **Mix in Extra Salt**

  Shuffle the eight star-marked expansion cards into the Sea Salt & Paper base-game deck. All base-game turn and end-round rules remain unchanged.

  Related topics: 
  - [Duo effects](#flow-duos)
  - [Special cards](#flow-specials)

- <!-- rule-item: flow-duos -->
  **Resolve new duo effects**

  Jellyfish plus Swimmer locks opponents to a top-deck draw next turn. Lobster plus Crab reveals five cards, lets you keep one, then returns and shuffles the other four.

  ![Jellyfish+Swimmer locks opponents; Lobster looks at 5](/images/rules/sea-salt-paper-extra-salt/new-duos.svg)

  Related topics: 
  - [Special cards](#flow-specials)
  - [Card counts](#flow-counts)

- <!-- rule-item: flow-specials -->
  **Choose a special-card payoff**

  Starfish turns a duo into a three-card, three-point set but cancels that duo's effect. Seahorse fills a missing collector card, while Crab Multiplier scores one point per other Crab.

  ![Starfish trio, Seahorse collector, Crab multiplier](/images/rules/sea-salt-paper-extra-salt/specials.svg)

  Related topics: 
  - [Card counts](#flow-counts)
  - [Back to setup](#flow-setup)

- <!-- rule-item: flow-counts -->
  **Know the eight new cards**

  The expansion adds two Jellyfish, one Lobster, three Starfish, one Seahorse, and one Crab Multiplier. The normal duo and STOP rules still apply.

  ![Use the count table; duos still 2; STOP still works](/images/rules/sea-salt-paper-extra-salt/counts.svg)

  Related topics: 
  - [Play the duos](#flow-duos)
  - [Back to setup](#flow-setup)

